...
function logUninstrumented() {
  if (uninstrumented.length > 0) {
    var message = 'The newrelic module must be the first module required.\n' +
                  'The following modules were required before newrelic and are ' +
                  'not being instrumented:';
...
function createMetrics(metrics) {
  ...
  uninstrumented.forEach(function addMetrics(module) {
    metrics.getOrCreateMetric(
      NAMES.SUPPORTABILITY.UNINSTRUMENTED + '/' + module
    ).incrementCallCount()
  })
}
...
function check() {
  for (var filename in require.cache) {
    if (require.hasOwnProperty(filename)) {
      return
    }
