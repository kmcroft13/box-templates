...
function generateMiddlewarePath(routerStack) {
  var res = '';
  for (var i = 0; i < routerStack.length; ++i) {
    var resLastChar = res[res.length - 1];
    var path = routerStack[i].path;
    if (resLastChar !== '/' && path[0] !== '/') {
      res += '/'
    }
    if (resLastChar === '/' && path[0] === '/') {
      res += path.substr(1)
...
function nameFromRoute(segment, route, params, append) {
  ...
  if (append) {
    ...
    trans.partialName += path[0] === '/' ? path.slice(1) : path
  } else {
    trans.partialName = NAMES.EXPRESS.PREFIX + trans.verb + NAMES.ACTION_DELIMITER;
    trans.partialName += path[0] === '/' ? path : '/' + path
  }
}
        ...
        return function wrappedLayerHandleError(error, req, res, next) {
          ...
          if (agent.config.feature_flag.express_segments) {
            ...
            if (routerStack.length > 0) {
              var parentRouter = routerStack[routerStack.length - 1];
              while (--parentRouter.length === 0) {
                parentRouter = routerStack.pop()
              }
              parentSegment = parentRouter.segment
            }
        ...
        return function wrappedLayerHandleRequest(req, res, next) {
          ...
          if (!res.end[ORIGINAL]) {
            ...
            res.end = function wrappedEnd() {
              ...
              var name = transaction.partialName;
              if (name && name[name.length - 1] === '/') {
                transaction.partialName = name.substring(0, name.length - 1)
              }
          ...
          var isMountedApp = (this.name === 'mounted_app' ||
                              this.handle.lazyrouter);
          var isRouter = (this.handle[ORIGINAL] && this.handle[ORIGINAL].stack);
          var isRoute = this.route;
          var stack = (this.handle[ORIGINAL] && this.handle[ORIGINAL].stack) ||
          ...
          if (agent.config.feature_flag.express_segments) {
            ...
            if (!isErrorHandler) {
              ...
              if (isMountedApp) {
                ...
                segmentName = NAMES.EXPRESS.PREFIX + 'Router: ' + this.handle.__NR_path
              } else if (isRoute) {
                ...
                var middlewarePath = generateMiddlewarePath(routerStack);
          ...
          function cleanup(err) {
            if (agent.config.feature_flag.express_segments) {
              if (!inRoute && !isMountedApp && !transactionInfo.responded &&
